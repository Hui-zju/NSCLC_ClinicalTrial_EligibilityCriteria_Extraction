    ...
    for i, l in enumerate(lines):
        ...
        for o in range(start, end):
            if o in offset_label:
                ...
                entity_start = offset_label[o].start
                # entity_end = offset_label[o].end
                break

        if label is not None:
            if label == prev_label and entity_start == old_entity_start:
                tag = 'I-' + label
            else:
                tag = 'B-' + label

        prev_label = label
        old_entity_start = entity_start
        # old_entity_end = entity_end
...
if __name__ == "__main__":
    ...
    # # process_files('../data/variation_re_dataset/variation_re_testing_dataset/NCT04181060_inc.txt')
    # # process_files(r'/home/chenh/PycharmProjects/clinicial_trial_variation_extraction/data/variation_re_dataset/variation_re_testing_dataset/NCT04181060_inc.txt')
    # # global options
    # options = argparser().parse_args()
    # fn = r'/home/chenh/PycharmProjects/clinicial_trial_variation_extraction/data/variation_re_dataset/variation_re_testing_dataset/NCT04181060_inc.txt'
    # with open(fn, 'rU') as f:
    #     lines = process(f)
